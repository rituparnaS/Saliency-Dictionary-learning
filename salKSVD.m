function [Dictionary,output] = salKSVD(...
    Data,... % an nXN matrix that contins N signals (Y), each of dimension n.
    param)
% =========================================================================
%                         (SDL) Saliency K-SVD algorithm
% =========================================================================
% The major part of this function is based on the K-SVD algorithm
% Paper: "The K-SVD: An Algorithm for 
...
%                                param.errorFlag = 0)
%    Dict                        : dictionary at each iteration 
%    err                         : reconstruction error at each iteration
...
for iterNum = 1:param.numIteration
    ...
    % if (param.errorFlag==0) % always run the saliency KSVD in error more
        CoefMatrix = OMP([FixedDictionaryElement,Dictionary],Data, param.L);
    % else 
        CoefMatrix = OMPerrSal([FixedDictionaryElement,Dictionary],Data, param.errorGoal,weight);
        param.L = 1;
    % end
    ...
    if (iterNum>1 & param.displayProgress)
        if (param.errorFlag==0)
            ...
            output.totalerr(iterNum-1) = sqrt(sum(sum(((weight*(Data-[FixedDictionaryElement,Dictionary]*CoefMatrix)')').^2))/prod(size(Data)));
    ...
    % If need to display dictioanry at each iteration
    % output.Dict{iterNum}    = Dictionary; 
	
    if (isfield(param,'waitBarHandle'))
        waitbar(iterNum/param.counterForWaitBar);
    end
    % If need to output reconstruction error at each iteration
    % output.err{iterNum}     = err_recon';
end
%
output.CoefMatrix = CoefMatrix;
Dictionary = [FixedDictionaryElement,Dictionary];
%output.err = sqrt(sum(sum(((weight*(Data-Dictionary*output.CoefMatrix)')').^2))/prod(size(Data)));
%output.indiverrwt = sqrt(sum(((weight*(Data-Dictionary*output.CoefMatrix)')').^2,1));
...
weights             = diag(W); % vector from diagonal matrix
wData               = (W*Data')';% same size as data
wj                  = weights(j);% weight for the spefic superpixel
...
tmpweights        = weights(relevantDataIndices);
